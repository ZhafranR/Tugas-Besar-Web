<?php
    ...
    $jenislangganan = query("SELECT *FROM jenisLangganan WHERE jenis='Basic'");
    ...
    if (isset($_POST["bayar"])) {
        ...
        if (bayar($_POST) > 0) {
            ...
            <script>
                alert('Berhasil!');
                document.location.href = 'Pembayaran/pembayaran-credit-card.php';
            </script>
            ";
        } else {
            echo "
            <script>
                alert('Kamu gagal !');
                document.location.href = 'pembayaran-basic.php';
            </script>
            ";
        }
    };
    function bayar($data)
    {
        ...
        $user = query("SELECT email FROM user");
        $jenisbasic= query("SELECT jenis FROM jenisLangganan WHERE jenis='Basic'");
        $hargabasic= query("SELECT harga FROM jenisLangganan WHERE jenis='Basic'");
        foreach ($user as $row) :
          $email = $row["email"]; 
        endforeach; 

        $query = "INSERT INTO langganan VALUES (' ','$email', '$jenisbasic', '$hargabasic', '$dateawal', '$dateselesai', '$kategori')";
        ...
        <div class="card">
          ...
          <div class="form-left">
            ...
            <?php foreach ($user as $row) : ?>
                  ...
                  ?>
            <?php endforeach; ?>
            <p class="f-email" name="email"><?php echo $email;?></p>
            <!-- Nama Paket -->
          ...
          <h3 class="form-text">Tanggal Selesai</h3>
                        ...
                        <p class="f-dateakhir" name="dateend"><?php
                          $tgl =  date('Y/m/d');
                          $dateselesai = manipulasiTanggal($tgl,'3','months');
                          echo $dateselesai;
                        ?></p>
                      </div>
          <button class="button" type="bayar" name="bayar" >Bayar</button>
          </from>
